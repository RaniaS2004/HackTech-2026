  ...
  return (
    <div className="rounded-xl border border-border bg-background/80 backdrop-blur-md">
      <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
        ...
        <span className="ml-2 font-mono text-xs text-muted-foreground">aurora://humanlock/attempts</span>
        <span className="ml-auto flex items-center gap-1.5 text-xs text-muted-foreground">
          <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-success" />
          live
        </span>
      </div>
      <pre className="cursor-blink whitespace-pre p-5 font-mono text-xs leading-relaxed text-foreground sm:text-sm">{typed}</pre>
    </div>
  );
}
