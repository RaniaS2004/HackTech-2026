      ...
      <div className="mx-auto max-w-6xl px-6">
        <div className="mx-auto mb-12 max-w-2xl text-center">
          <div className="mb-3 inline-flex items-center gap-2">
            ...
            <span className="text-xs font-medium uppercase tracking-widest text-primary">AgentPass</span>
            <span className="h-px w-6 bg-primary" />
          </div>
          <h2 className="text-balance text-4xl font-semibold tracking-tight sm:text-5xl">Signed identity for legitimate agents.</h2>
          ...
          <div className="relative flex flex-col gap-4 rounded-xl border border-border bg-background/60 p-5 backdrop-blur-sm">
            ...
            <div className="rounded-lg border border-border bg-card p-3 font-mono text-[11px] leading-relaxed">
              ...
              <div className="rounded-lg border border-border bg-background/70 p-3 text-foreground">
                <div><span className="text-muted-foreground">source:</span> {challenge?.source ?? "loading"}</div>
                <div className="mt-2"><span className="text-muted-foreground">challenge:</span></div>
                <pre className="mt-1 overflow-auto whitespace-pre-wrap text-[11px] leading-relaxed text-primary">{challenge?.challenge ?? "Loading challenge..."}</pre>
            ...
            {result?.passed ? (
              <div className="rounded-lg border border-success/30 bg-success/10 p-3">
                ...
                <div className="mt-3 space-y-2 font-mono text-[11px] text-muted-foreground">
                  <div><span className="text-foreground">JWT:</span> {result.token ? "issued" : "not issued"}</div>
                  <div><span className="text-foreground">signature:</span> {result.solana?.signature ?? "pending"}</div>
                </div>
                {result.solana?.explorer_url ? (
                  <a href={result.solana.explorer_url} target="_blank" rel="noreferrer" className="mt-3 inline-block text-xs text-primary underline">
