...
export function HumanLockDemo() {
  ...
  const loadChallenge = useCallback(async (resetRecovery = true) => {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= IMAGE_CHALLENGE_RETRY_LIMIT; attempt += 1) {
        ...
        if (!response.ok) {
          throw new Error(`HumanLock challenge failed with ${response.status}`);
        }

        data = (await response.json()) as HumanChallenge;
        if (data.type === "image") break;
        if (attempt < IMAGE_CHALLENGE_RETRY_LIMIT) {
          await delay(250);
        }
      }

      if (!data) {
        throw new Error("HumanLock challenge returned no data");
      }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`HumanLock verify failed with ${response.status}`);
      }
    ...
    <section id="humanlock" className="relative py-28">
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div
          ...
          className="relative mx-auto max-w-md rounded-2xl border border-border bg-card/40 p-6 backdrop-blur-md"
        >
              ...
              {challenge?.type === "image" ? (
                <>
                  <p className="mb-3 text-xs text-muted-foreground">
                    Select the correct label for this adversarial image challenge.
                  </p>
                  <img
                    src={`data:image/png;base64,${challenge.image_b64}`}
                    alt="HumanLock challenge"
                    className="w-full rounded-xl border border-border bg-black object-cover"
                  />
                      ...
                      <button
                        ...
                        className={`rounded-md border px-3 py-2 text-left text-xs font-medium transition-colors ${
                          answer === option
                            ...
                            : "border-border bg-background/70 text-foreground hover:bg-secondary/70"
                        }`}
                      >
                        {option}
                      </button>
